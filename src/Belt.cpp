...
Belt::Belt(std::string className)
    : SimulatedComponent(className), prev(nullptr), next(nullptr), lanes{} {}
...
bool Belt::willPushItem() const {
    for (unsigned l = Lane::LEFT; l < Lane::COUNT; ++l) {
        if (lanes[l][LANE_LENGTH - 1] != nullptr)
            return true;
    }
    return false;
}
...
Belt *Belt::getNext() const { return next; }

Belt *Belt::getPrev() const { return prev; }
