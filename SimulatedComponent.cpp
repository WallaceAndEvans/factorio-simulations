...
Lane::Type operator+=(Lane::Type &l, int r) {
    ...
    if (newLane < 0 || newLane > Lane::COUNT) {
        ...
        ss << "lane " << newLane << " is out of range [0," << static_cast<int>(Lane::COUNT) << ']';
...
SimulatedComponent::SimulatedComponent(std::string name): name(getUniqueName(name)), simulated(UINT_MAX) {}
...
bool SimulatedComponent::finishedStep(unsigned time) {
    if (simulated == UINT_MAX) return false;
...
void SimulatedComponent::step(unsigned time) {
    if (finishedStep(time)) {
        std::ostringstream ss;
        ss << "timestep " << time << " has already been visited for " << getName();
